...
DB = DataBase(
    user=[
        User(username="qa1@1kcompfox.com", hashed_password=crypto.hash("compfox1")),
        User(username="qa2@2kcompfox.com", hashed_password=crypto.hash("compfox2")),
        User(username="qa3@3kcompfox.com", hashed_password=crypto.hash("compfox3")),
        User(username="qa4@4kcompfox.com", hashed_password=crypto.hash("compfox4")),
...
@app.post("/work", response_class=HTMLResponse)
async def index_work(request: Request, background_tasks: BackgroundTasks):
    d={}
    if not user:
    ...
    # Upload the JSON data to the GCP bucket
    send_bucket(data)

    # Render the HTML template
    context = {"request": request, "data": data}
    return templates.TemplateResponse("just_simple_test.html", context)
...
@app.post('/process_html', response_class=HTMLResponse)
async def process_html(request: Request, id: str,json_data: str ):
